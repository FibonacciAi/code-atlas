...
Graph uses bundled Mermaid 11.12.2, entirely offline. A cancellable background scan reads every eligible file in the indexed workspace, up to the existing 4 MiB per-file limit. It stores relationships without retaining source text. Search, selection, and display pages reuse that index; they do not limit the scan. Python relative imports, multiline imports, and `src/` layouts are supported. Import candidates remain lexical evidence, including conditional and function-local imports, not compiler-resolved runtime dependencies.

Large graphs open as folder summaries. Click a folder or use the folder picker to drill down, and Back to go up. More than 100 items at one level use Previous/Next pages. The footer distinguishes scan coverage from relationships outside the current display page; folder summaries combine edges and hide intra-folder detail until you drill in. Selection + neighbors shows a file's immediate connections. Unreadable files and excluded/non-text files are counted explicitly. The indexer's file-count and source-policy limits still apply.
